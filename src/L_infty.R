...
L_infty_is = function(t_0, T, Y_hat_is_list, Yhat_LASSO_is_list, Y_hat_is_list_NN, Y_hat_PLS_is_list, y_train_list, dates) {
  ...
  for (i in 1:T){
    ...
    l_inf.vect_is[i] <- max(yhat_is - y_train_list[[t_0+i]])
    l_inf_LASSO.vect_is[i] <- max(yhat_is_LASSO - y_train_list[[t_0+i]])
    l_inf_NN.vect_is[i] <- max(yhat_is_NN - y_train_list[[t_0+i]])
    l_inf_PLS.vect_is[i] <- max(yhat_is_PLS - y_train_list[[t_0+i]])
  }
  ...
  ggplot(df.toplot_linf, aes(x)) + 
    geom_line(aes(y=l_inf.vect_is, color="1"), size=0.8) + 
    geom_line(aes(y=l_inf_LASSO.vect_is, color="2"), size=0.8) +
    geom_line(aes(y=l_inf_NN.vect_is, color="3"), size=0.8) +
    geom_line(aes(y=l_inf_PLS.vect_is, color="4"), size=0.8) +
  ...
  ggsave(file="../png/In_sample_Linfty_tuned.png", width=10, height=5, dpi=400)
  ggsave(file="../pdf/In_sample_Linfty_tuned.pdf", width=10, height=5)
}
...
L_infty_oos = function(t_0, T, Y_hat_oos_list, Yhat_LASSO_oos_list, Y_hat_oos_list_NN, Y_hat_PLS_oos_list, y_test_list, dates) {
  ...
  for (i in 1:T){
    ...
    l_inf.vect_oos[i] <- max(yhat_oos - y_test)
    l_inf_NN.vect_oos[i] <- max(yhat_oos_NN - y_test)
    l_inf_LASSO.vect_oos[i] <- max(yhat_oos_LASSO - y_test)
    l_inf_PLS.vect_oos[i] <- max(yhat_oos_PLS - y_test)
  }
  ...
  df.toplot_linf <- data.frame(x)
  ggplot(df.toplot_linf, aes(x)) + 
    geom_line(aes(y=l_inf.vect_oos, color="1"), size=0.8) + 
    geom_line(aes(y=l_inf_LASSO.vect_oos, color="2"), size=0.8) +
    geom_line(aes(y=l_inf_NN.vect_oos, color="3"), size=0.8) +
    geom_line(aes(y=l_inf_PLS.vect_oos, color="4"), size=0.8) +
    scale_colour_manual(name = "Legend", 
                        values=c("cadetblue3","tomato", "orange", "green"), 
  ...
  ggsave(file="../png/Out_of_sample_Linfty_tuned.png", width=10, height=5, dpi=400)
  ggsave(file="../pdf/Out_of_sample_Linfty_tuned.pdf", width=10, height=5)
}